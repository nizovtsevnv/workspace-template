...
if [ "$STATUS" = "не инициализирован" ]; then
	commands_data="make init<COL>Инициализация проекта из шаблона<ROW>make versions<COL>Показать версии инструментов технологических стеков<ROW>make modules create<COL>Создать новый модуль (Node.js, PHP, Python, Rust)<ROW>make modules import<COL>Импортировать модуль из git репозитория<ROW>make modules pull<COL>Обновить workspace и все субмодули<ROW>make modules push<COL>Отправить изменения субмодулей и workspace<ROW>make modules sync<COL>Синхронизировать в обоих направлениях (pull + push)<ROW>make modules status<COL>Статус всех субмодулей<ROW>make template status<COL>Показать статус и версию шаблона<ROW>make template update<COL>Обновить версию шаблона<ROW>make template test<COL>Запустить автотесты шаблона"
else
	commands_data="make versions<COL>Показать версии инструментов технологических стеков<ROW>make modules create<COL>Создать новый модуль (Node.js, PHP, Python, Rust)<ROW>make modules import<COL>Импортировать модуль из git репозитория<ROW>make modules pull<COL>Обновить workspace и все субмодули<ROW>make modules push<COL>Отправить изменения субмодулей и workspace<ROW>make modules sync<COL>Синхронизировать в обоих направлениях (pull + push)<ROW>make modules status<COL>Статус всех субмодулей<ROW>make template status<COL>Показать статус и версию шаблона<ROW>make template update<COL>Обновить версию шаблона<ROW>make template test<COL>Запустить автотесты шаблона"
fi
