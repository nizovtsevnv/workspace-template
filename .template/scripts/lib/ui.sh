...
# Показать индикатор прогресса (статичный)
# Параметры: $1 - сообщение
# Использование: log_progress "Обновление модуля..."
log_progress() {
	printf "${COLOR_DIM}⠿${COLOR_RESET} %s\r" "$1" >&2
}

# Очистить строку прогресса (для замены результатом)
# Использование: clear_progress
clear_progress() {
	if is_tty; then
		printf "\r\033[K" >&2
	fi
